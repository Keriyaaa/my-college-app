...
export default function DigitalMainScreen({ navigation }: any) {
  ...
  const loadPosts = async () => {
    try {
      const savedPosts = await AsyncStorage.getItem('digital_posts');
      if (savedPosts) {
        ...
        const initialPosts: Post[] = [
          {
            ...
            content: 'Сегодня на лекции по веб-разработке разбирали React Native! Очень интересная технология для создания мобильных приложений.',
            likes: 15,
            comments: [
              { 
                id: '1', 
                ...
                text: 'Круто! Жду следующих занятий', 
          ...
          },
          {
            id: '2',
            user: 'Максим Иванов',
            userEmail: 'maxim@edu.kait20.ru',
            content: 'Завтра практическое занятие по Figma. Не забудьте установить программу!',
            likes: 8,
            comments: [],
            isLiked: false,
        ...
        await AsyncStorage.setItem('digital_posts', JSON.stringify(initialPosts));
      }
    } catch (error) {
  ...
  const savePosts = async (updatedPosts: Post[]) => {
    try {
      await AsyncStorage.setItem('digital_posts', JSON.stringify(updatedPosts));
    } catch (error) {
  ...
  // Обновленный выпадающий список
  const dropdownOptions = [
    { 
    id: '1', 
    title: 'Дипломная работа', 
    action: () => navigation.navigate('ThesisSupervisors') // Изменено!
  },
    { id: '2', title: 'Учебный план', action: () => Alert.alert('Учебный план', 'Расписание и учебные материалы') },
    { 
    id: '3', 
    title: 'Партнеры', 
    action: () => navigation.navigate('Partners') // Изменено!
  },
    // { id: '4', title: 'Столовая', action: () => Alert.alert('Столовая', 'Меню и график работы') },
    { id: '5', title: 'Отделения', action: () => navigation.navigate('Department') },
    { 
    id: '6', 
    title: 'Устав', 
    action: () => navigation.navigate('Charter') // Изменено!
  },
];
