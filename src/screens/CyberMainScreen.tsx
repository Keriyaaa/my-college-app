...
export default function CyberMainScreen({ navigation }: any) {
  ...
  const loadPosts = async () => {
    try {
      const savedPosts = await AsyncStorage.getItem('cyber_posts');
      if (savedPosts) {
        setPosts(JSON.parse(savedPosts));
      } else {
        // Начальные данные для Cyber
        const initialPosts: Post[] = [
          {
            id: '1',
            user: 'Алексей Петров',
            userEmail: 'alexey@edu.kait20.ru',
            content: 'Сегодня на практике разбирали методы защиты от DDoS-атак. Очень полезная информация для будущих специалистов по кибербезопасности!',
            likes: 15,
            comments: [
              { 
                id: '1', 
                ...
                text: 'Какие инструменты для мониторинга трафика рекомендуете?', 
          ...
          },
          {
            ...
            content: 'Подготовил материал по основам криптографии для новичков. Кому интересно - подходите, поделюсь конспектами и полезными ресурсами.',
            likes: 22,
            comments: [
              { 
                id: '2', 
                ...
                text: 'Сергей, спасибо за материалы! Очень помогли разобраться с RSA.', 
          ...
          },
          {
            ...
            content: 'Ищем участников для CTF-соревнований по взлому и защите веб-приложений. Опыт не обязателен - научим! Пишите в личные сообщения.',
            likes: 31,
            comments: [
              { 
                id: '3', 
                user: 'Антон Никитин', 
                userEmail: 'anton@edu.kait20.ru',
                text: 'Я готов участвовать! Есть базовые знания по Python и сетям.', 
              ...
              { 
                ...
                text: 'Когда планируете первые тренировки?', 
                timestamp: '2 часа назад' 
              },
            ],
            isLiked: false,
          ...
          },
          {
            ...
            content: 'Прошел сертификацию по этичному хакингу. Если есть вопросы по пентесту или поиску уязвимостей - обращайтесь, помогу разобраться.',
            likes: 18,
            comments: [],
            isLiked: false,
          ...
          },
          {
            ...
            content: 'Нашла интересный кейс по анализу вредоносного ПО. Кто хочет вместе разобрать механизмы работы трояна - присоединяйтесь к исследованию.',
            likes: 27,
            comments: [
              { 
                id: '5', 
                ...
                text: 'Ольга, я бы с удовольствием поучаствовал! Есть опыт с анализом в Sandbox.', 
        ...
        await AsyncStorage.setItem('cyber_posts', JSON.stringify(initialPosts));
      }
    } catch (error) {
  ...
  const savePosts = async (updatedPosts: Post[]) => {
    try {
      await AsyncStorage.setItem('cyber_posts', JSON.stringify(updatedPosts));
    } catch (error) {
  ...
  // Выпадающий список для Cyber
  const dropdownOptions = [
   { 
    id: '1', 
    title: 'Дипломная работа', 
    action: () => navigation.navigate('CyberThesisSupervisors')
  },
  { 
    id: '2', 
    title: 'Учебный план', 
  ...
  { 
    id: '3', 
    title: 'Партнеры', 
    action: () => navigation.navigate('CyberPartners')
  },
  { 
    id: '5', 
    title: 'Отделения', 
  ...
  { 
    id: '6', 
    title: 'Устав', 
    action: () => navigation.navigate('CyberCharter')
  },
  ];
